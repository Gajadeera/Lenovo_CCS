...
const CreateJob = ({ isOpen, onClose, onCreateJob }) => {
    ...
    const handleFileChange = (e) => {
        ...
        try {
            ...
            const totalFiles = formData.documents.length + files.length;
    ...
    const removeFile = useCallback((id) => {
        setFormData(prev => {
            const fileIndex = filePreviews.findIndex(file => file.id === id);
        ...
        setFilePreviews(prev => {
            ...
            const [removedFile] = newPreviews.splice(fileIndex, 1);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            formDataToSend.append('customer', formData.customer_id);
            formDataToSend.append('serial_number', formData.serial_number);
            if (formData.device_id) {
                formDataToSend.append('device', formData.device_id);
            }
            ...
            formDataToSend.append('warranty_status', formData.warranty_status);

            formData.documents.forEach(file => {
                formDataToSend.append('documents', file);
            });

            const response = await axios.post('http://localhost:5000/jobs', formDataToSend, {
                headers: {
                    ...getAuthHeaders(),
            ...
            if (response.data.success) {
                if (onCreateJob) {
                    onCreateJob(response.data.data);
                }
                        ...
                        <div className="col-span-4 md:col-span-3 flex items-center">
                            ...
                            <label className="inline-flex items-center p-2 rounded-md bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 cursor-pointer transition-colors">
                                ...
                                <input
                                    ...
                                    className="hidden"
                                    multiple
                                    disabled={formData.documents.length >= MAX_FILES || loading}
                                />
